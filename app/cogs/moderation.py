...
class Moderation(commands.Cog, name="moderation"):
    def __init__(self, bot) -> None:
        self.bot = bot
        self.service = bot.services.moderation
    ...
    async def kick(
        ...
        if member.guild_permissions.administrator:
            embed = discord.Embed(
                description="User has administrator permissions.", color=EmbedColor.ERROR
            )
    ...
    async def warning_add(
        ...
        total = await self.service.warn_user(
            user_id=user.id,
    ...
    async def warning_remove(
        ...
        total = await self.service.remove_warn(
            warn_id=warn_id, user_id=user.id, server_id=context.guild.id
        )
        embed = discord.Embed(
    ...
    async def warning_list(self, context: Context, user: discord.User) -> None:
        warnings = await self.service.list_warnings(
            user_id=user.id, server_id=context.guild.id
        )
        embed = discord.Embed(title=f"Warnings of {user}", color=EmbedColor.PRIMARY)
        if not warnings:
            embed.description = "This user has no warnings."
        else:
            embed.description = "\n".join(
                f"• Warned by <@{w.moderator_id}>: **{w.reason}** "
                f"(<t:{int(w.created_at.timestamp())}>) - Warn ID #{w.warn_id}"
                for w in warnings
            )
        await context.send(embed=embed)
